...
def compute_clip_features_batched(
    ...
    if prompt_h is not None and prompt_w is not None:
        ...
        scale_w, scale_h = 1, 1  # No scaling if prompt dimensions are not provided
    ...
    preprocessed_images_batch = torch.cat(preprocessed_images, dim=0).to(device)  # num_of_detected_objects x 3 x 224 x 224
    text_tokens_batch = clip_tokenizer(text_tokens).to(device)

    # Batch inference
    with torch.no_grad():
        image_features = clip_model.encode_image(preprocessed_images_batch)  # Encodes the cropped images with CLIP
