"""
Object-goal exploration: instead of giving the agent an image of the target (image-goal
task_type in GOAT-Bench) or relying on the benchmark's pre-annotated object-goal episodes,
this script takes a free-typed word (e.g. "sofa", "refrigerator") on the command line and
lets the agent explore the scene to find it.

Because the word is not necessarily one of this demo scene's annotated GOAT-Bench object
instances, there is no ground-truth position/viewpoint to score against - this script does
not report success rate / SPL. It reports the trajectory and the VLM's own final answer
(which Snapshot/Object it settled on), same as the visualization folders produced by
...
def main(cfg, object_word, split=1):
    ...
    # this demo dataset only ships one scene file; reuse it purely for the scene id and
    # the episode's start pose - its "tasks"/"goals" ground truth is not used here
    ...
    subtask_id = f"{scene_id}_{episode_id}_objectgoal_{object_word.replace(' ', '_')}"
    episode_dir, eps_frontier_dir, eps_snapshot_dir, eps_potential_dir = logger.init_episode(
        episode_id=f"{scene_id}_ep_{episode_id}_objectgoal"
    )
    ...
    logging.info(f"\nObject-goal subtask: find '{object_word}'")

    # manually build subtask_metadata: there is no GOAT-Bench annotated instance for a
    # free-typed word, so goal_obj_ids / viewpoints / gt_subtask_explore_dist stay empty
    # (this is the same shape Logger.init_subtask() builds for goal_type == "object",
    # just without ground-truth fields)
    subtask_metadata = {
        "question_id": subtask_id,
        "question": f"Can you find the {object_word}?",
        "image": None,
        "answer": object_word,
        "goal_obj_ids": [],
        "class": object_word,
    ...
    # replicate the minimal per-subtask bookkeeping Logger.init_subtask() normally does
    ...
    goal_obj_ids_mapping = {}  # no ground-truth ids to track for a free-typed word
    ...
    while cnt_step < num_step - 1:
        ...
        if type(max_point_choice) == SnapShot and target_arrived:
            ...
            rgb = obs["color_sensor"]
            plt.imsave(
                os.path.join(logger.subtask_object_observe_dir, "target.png"), rgb
            )
    ...
    # Save final potential graph state
    if cfg.save_visualization:
    ...
    # no ground-truth position exists for a free-typed word, so there is no success/SPL
    # to compute - report the VLM's own outcome and the trajectory length instead
    result_summary = {
        ...
        "chosen_snapshot": max_point_choice.image
        if task_success and isinstance(max_point_choice, SnapShot)
        else None,
    }
    with open(
        os.path.join(episode_dir, f"object_goal_result_{object_word.replace(' ', '_')}.json"),
        "w",
    ) as f:
        json.dump(result_summary, f, indent=2)

    logging.info(f"Object-goal result: {result_summary}")
    scene.print_scene_graph()

    logging.info("Object-goal run finished")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-cf", "--cfg_file", help="cfg file path", default="cfg/eval_object_goal.yaml", type=str)
    parser.add_argument("--object_word", help="the object word to search for, e.g. 'sofa' or 'refrigerator'", required=True, type=str)
    ...
    word_slug = args.object_word.replace(" ", "_")
    logging_path = os.path.join(
        str(cfg.output_dir),
        f"log_objectgoal_{word_slug}_{args.split}.log",
    )
    ...
    logging.info(f"***** Running {cfg.exp_name}: object-goal '{args.object_word}' *****")
    main(cfg, object_word=args.object_word, split=args.split)
