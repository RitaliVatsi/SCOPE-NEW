...
selection toward it. Whenever the currently detected object classes in the scene graph
match the current landmark, or the agent navigates to a chosen snapshot for it, that
landmark is marked "found" in a running mental-map state and the agent moves on to the
next landmark in the chain. Only reaching the final entry (the actual target) ends the
run successfully.
...
def detect_landmark_in_scene(scene, landmark_name):
    """Fuzzy-match the current landmark name against the class names of objects
    already in the scene graph. Returns the matching obj_id, or None."""
    landmark_name = landmark_name.lower()
    for obj_id, obj in scene.objects.items():
        class_name = obj["class_name"].lower()
        if landmark_name in class_name or class_name in landmark_name:
            return obj_id
    return None
...
def main(cfg, chain_prompt, split=1):
    ...
    while cnt_step < num_step - 1:
        ...
        # passively check if the current landmark already shows up in the scene graph,
        # even before the agent explicitly navigates to it
        matched_obj_id = detect_landmark_in_scene(scene, active_entry["name"])
        if matched_obj_id is not None and not active_entry["found"]:
            active_entry["found"] = True
            active_entry["found_step"] = global_step
            logging.info(f"Mental map: landmark '{active_entry['name']}' detected in scene graph at step {global_step} (obj_id {matched_obj_id})")
            if active_entry["is_target"]:
                task_success = True
                break
            active_entry = current_active_entry(mental_map)
            logging.info(f"Advancing to next landmark: '{active_entry['name']}'")
            # reset navigation target so the VLM is queried fresh for the new landmark
